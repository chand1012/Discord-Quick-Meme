...
func getABuzzWord() string {
	...
	adverbs := []string{"appropriately", "assertively", "authoritatively", "collaboratively", "compellingly", "competently", "completely",
	...
	verbs := []string{"actualize", "administrate", "aggregate", "architect", "benchmark", "brand", "build", "communicate", "conceptualize",
	...
	adjectives := []string{"24/7", "24/365", "accurate", "adaptive", "alternative", "an expanded array of", "B2B", "B2C", "backend",
	...
	nouns := []string{"action items", "alignments", "applications", "architectures", "bandwidth", "benefits",
	...
	adverbsIndex := rand.Intn(len(adverbs))
	verbsIndex := rand.Intn(len(verbs))
	adjectivesIndex := rand.Intn(len(adjectives))
	nounsIndex := rand.Intn(len(nouns))

	statement := adverbs[adverbsIndex] + " " + verbs[verbsIndex] + " " + adjectives[adjectivesIndex] + " " + nouns[nounsIndex] + "."

	return strings.Title(statement)
}
